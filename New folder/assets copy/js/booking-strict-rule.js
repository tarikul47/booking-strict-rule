jQuery(document).ready(function ($) {
    ...
    function handlePickupDateSelection() {
        ...
        if (!isAvailable) {
            ...
            alert(`The next ${daysToCheck} calendar days are not available. Please choose another pickup date.`);
    ...
    function initializeInventoryRule(inventoryId) {
        if (BOOKING_STRICT_RULE.inventory_rules[inventoryId]) {
            activeRuleDays = parseInt(BOOKING_STRICT_RULE.inventory_rules[inventoryId].days, 10);
    ...
    $('#rnb-inventory-list').on('change', function() {
    ...
    const initialInventoryId = $('#rnb-inventory-list').val();
